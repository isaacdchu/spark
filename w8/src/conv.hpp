...
public:
    ...
    Conv2dTinydnn(
        ...
        // tiny-dnn's AVX backend (enabled by -DCNN_USE_AVX, which the build sets
        // whenever the target has AVX) dispatches on kernel size alone:
        // conv2d_op_avx.h routes every 5x5 convolution to avx_conv2d_5x5_kernel,
        // which silently ignores w_dilation/h_dilation and so computes the wrong
        // result for dilated 5x5. Fall back to the scalar `internal` engine in
        // that case; every other shape either has no AVX path or is unaffected.
        const tiny_dnn::core::backend_t engine =
            (R == 5 && S == 5 && (dilation[0] != 1 || dilation[1] != 1))
                ? tiny_dnn::core::backend_t::internal
                : tiny_dnn::core::default_engine();
