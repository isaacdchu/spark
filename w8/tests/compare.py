...
def compare_sections(expected: list[Section], actual: list[Section], backend: str) -> list[str]:
    ...
    for expected_section, actual_section in zip(expected, actual):
        ...
        for value_index, (expected_value, actual_value) in enumerate(zip(expected_section.values, actual_section.values)):
            # float32 accumulates in a different order than PyTorch (SIMD FMA vs
            # reference), so use a combined absolute + relative tolerance: the
            # absolute term covers values near zero, the relative term scales
            # with magnitude for large accumulations (many channels/taps).
            if abs(expected_value - actual_value) > 1e-4 + 1e-5 * abs(expected_value):
