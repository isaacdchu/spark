...
# arm64 (macOS) and on pre-AVX x86. Mirrors TINYDNN_AVX in the Makefile.
TINYDNN_AVX=()
if echo | "${CXX}" -march=native -dM -E - 2>/dev/null | grep -q '__AVX__'; then
  TINYDNN_AVX=(-DCNN_USE_AVX)
fi
...
for config in "${BLOCK_CONFIGS[@]}"; do
  ...
  if [[ "${BENCH_CMD}" == "./${BENCH_BIN}" ]]; then
    ...
    # LDFLAGS go last: GNU ld resolves archive symbols in a single left-to-right
    # pass, so the libraries must follow the translation unit that needs them.
    COMPILE_CMD=("${CXX}" "${CXXFLAGS[@]}" "${DEFINES[@]}" "${INCLUDE[@]}" -I"${SOURCE}" "${BENCH_SRC}" -o "${BENCH_BIN}" "${LDFLAGS[@]}")
    echo "compiling:"
    echo "  ${COMPILE_CMD[*]}"
    # Tee compiler stdout+stderr into the log (as the run step does) so the
    # dashboard sees the real error on failure. Capture the compiler's own exit
    # status via PIPESTATUS[0] -- tee's success must not mask a failing compile.
    set +e
    "${COMPILE_CMD[@]}" 2>&1 | tee -a "${LOG_FILE}"
