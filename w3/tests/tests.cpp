...
static std::string run_case(const TestCase& conv_case) {
...
TestCase make_test_case(
    ...
    for (std::size_t i = 0; i < product(input_shape); i++) {
        input_values.push_back(std::sin(input_seed + static_cast<float>(i)) * 10.0f);
    }
    std::vector<float> kernel_values = std::vector<float>();
    for (std::size_t i = 0; i < product(kernel_shape); i++) {
        kernel_values.push_back(std::cos(kernel_seed + static_cast<float>(i)) * 10.0f);
    }
    return TestCase(
